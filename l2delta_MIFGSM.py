...
checkpoint = torch.load(f'./models/trained/mobilenetv2.pth.tar')
...
for k in tqdm(range(0, num_batches)):
    ...
    for t in tqdm(range(max_iterations)):
        ...
        for j in range(batch_size_cur):
            delta.data[j] = delta.data[j] + lr * grad_a[j] / torch.linalg.norm(grad_a[j])
...
np.save('delta_l2.npy', adv_img.astype(np.int32) - images.astype(np.int32))
