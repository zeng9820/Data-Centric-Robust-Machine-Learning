import numpy as np
#imaug库https://github.com/aleju/imgaug
...
data_aug5 = np.clip(test_data.astype(np.int32) + delta_linf + delta_l2, 0, 255).astype(np.uint8)#将l2和linf扰动叠加
#data_aug5 = np.clip(test_data.astype(np.int32) + delta_l2, 0, 255).astype(np.uint8)
#data_aug5 = np.clip(test_data.astype(np.int32) + delta_linf , 0, 255).astype(np.uint8)
